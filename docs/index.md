...
- 系统渠道的 `modelKey` 是前台选择、能力校验和计费使用的产品 SKU；`providerModelKey` 才是发送给上游的真实模型 ID，空值仅兼容为 `modelKey`。
- 每个公开前台逻辑模型必须只路由到同名渠道 SKU；能力参数与 Token 价格要和该渠道模型一致，不能按上游模型 ID 重新归并。

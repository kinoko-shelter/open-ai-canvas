...
本文档说明本项目二次开发时的 Git 远程仓库、分支和上游同步。服务器操作见 [DEPLOYMENT.md](DEPLOYMENT.md)。
...
当前团队服务器只部署 `custom/main`，使用裸机 systemd 方案。日常入口：
...
完整流程、重启语义、回退、飞书通知和排障命令统一维护在 [DEPLOYMENT.md](DEPLOYMENT.md)，本文件不再重复。

                        ...
                        <Form.Item name="mode" label="存储类型" rules={[{ required: true, message: "请选择存储类型" }]}>
                            <Segmented<StorageMode>
                                ...
                                onChange={(value) => {
                                    ...
                                    if (switchingProvider) form.setFieldsValue({ region: "", endpoint: "", cdnBaseUrl: "", cdnAuthKey: "", bucket: "", accessKeyId: "", accessKeySecret: "" });
                                }}
                            />
                        </Form.Item>
                        ...
                        {isObjectStorage ? (
                            <div className="space-y-1">
                                ...
                                <div className="grid gap-x-4 gap-y-1 md:grid-cols-2">
                                    ...
                                    {mode === "aliyun" ? (
                                        <Form.Item name="cdnAuthKey" label={setting?.hasCdnAuthKey ? `CDN Type A 鉴权密钥（${configuredSecretText}）` : "CDN Type A 鉴权密钥"}>
                                            <Input.Password autoComplete="new-password" placeholder={setting?.hasCdnAuthKey ? "留空保留原密钥" : "在 CDN 控制台启用 Type A 后填写"} />
                                        </Form.Item>
                                    ) : null}
                                </div>
                            </div>

...
export function assetStorageUsageQueryKey(scope = getActiveUserScope()) {
    return ["account-file-storage-usage", scope] as const;
}

export function AssetStorageUsage() {
    const scope = getActiveUserScope();
    const query = useQuery({
        queryKey: assetStorageUsageQueryKey(scope),
    ...
    const full = Boolean(usage && usage.totalBytes > 0 && usage.usedBytes >= usage.totalBytes);

    return (
        <section className={`assets-storage-usage${full ? " is-full" : ""}${usage?.usedBytes ? " has-usage" : ""}`} aria-label="账号文件容量" aria-busy={query.isPending} title="统计服务端资源和 Agent 会话附件；删除素材不会自动清理已上传对象">

...
import { useUserStore, type FeatureAvailability, type LocalUser } from "@/stores/use-user-store";
            ...
            >
                <span className="flex min-w-0 items-center gap-3">
                    ...
                    <span className="flex min-w-0 flex-col">
                        <span className="app-workspace-brand-wordmark truncate text-[var(--fs-body)] leading-none font-medium">故事创作</span>
                ...
                <>
                    ...
                    <div className="app-workspace-nav-popover absolute left-3 right-3 top-full z-50 mt-1 overflow-hidden rounded-lg border border-[var(--workspace-border)] bg-[var(--workspace-surface-strong)] py-1 animate-in fade-in zoom-in-95 duration-100">
                        <div className="px-3 py-2.5">
                            <div className="truncate text-[var(--fs-body)] font-semibold">故事创作</div>
...
export function WorkspaceSidebarNav({ onNavigate, onOpenSearch }: { onNavigate: () => void; onOpenSearch: () => void }) {
    ...
    const { groups, footer } = useMemo(() => buildNav(features, balance, user), [features, balance, user]);

...
- 方舟 Seedance 的参考图片可在后台“方舟素材库”配置 IAM AK/SK 后同步到方舟私域虚拟人素材库。仅当前用户可读取的素材库图片会进入该流程；服务端缓存审核通过的 `asset://` ID，审核失败不会回退为普通图片 URL。AK/SK 仅加密保存在服务端，IAM 账号应只授予必要的 `ark:*Asset*` 权限。

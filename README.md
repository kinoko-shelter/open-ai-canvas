<p align="center">
  <img src="web/public/logo.svg" width="88" alt="故事创作 logo">
</p>

<h1 align="center">故事创作</h1>
...
一个故事也许始于一页小说、一个人物，或一句还没写完的对白。故事创作从章节中梳理角色与情节，让人物的外观、声音和气质成为可复用的角色资产，再把分镜、图片、视频和音频组织在同一张画布上。从最初的文字到可以被看见、被听见的镜头，创作者始终掌握故事的方向。

故事创作是一款面向 AI 影视与短剧创作的开源工作台，集成自由画布、结构化分镜、角色卡、3D 导演台、素材库、异步生成任务和 Agent 协作能力。
...
<p align="center">
  <img src="assets/qq.jpg" alt="故事创作 QQ 交流群" width="360">
</p>
...
- 方舟 Seedance 的参考图片可在后台“方舟素材库”配置 IAM AK/SK 后同步到方舟私域虚拟人素材库。画布图片节点的“更多 → 上传到方舟素材库”可提前完成同步；若图片尚未进入系统素材库，会先按当前用户和团队权限保存，再提交方舟审核。服务端缓存审核通过的 `asset://` ID，审核失败不会回退为普通图片 URL。AK/SK 仅加密保存在服务端，IAM 账号应只授予必要的 `ark:*Asset*` 权限。

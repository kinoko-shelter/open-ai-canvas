...
func (r *Repository) AdminUserTasks(userID string, limit int, offset int) ([]model.Task, int64, error) {
	...
	err := query.Select("id", "user_id", "session_id", "project_id", "aigc_project_id", "type", "status", "stage", "progress", "operation", "provider", "model", "billing_order_id", "provider_request_id", "poll_stage", "attempts", "started_at", "completed_at", "created_at", "updated_at").
